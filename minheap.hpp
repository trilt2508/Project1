...
typedef struct Car_Terrain {
	int teta;
	int x;
	int y;
	struct Car_Terrain* parent;
	double gx;
	double hx;
	double fx;
}Car_Terrain;
...
private:
	...
	void heapify_down(int i)
	{
		...
		if (left < size() && A[left]->fx < A[i]->fx)
			smallest = left;

		if (right < size() && A[right]->fx < A[smallest]->fx)
			smallest = right;
